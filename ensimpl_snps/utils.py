...
def bins(start, stop, fmt='gff', one=True):
    ...
    Args:
        ...
        fmt (str, optional): Either ``'gff'`` or ``'bed'``. This specifies
            1-based start coords (gff) or 0-based start coords (bed).
        one (bool):  If ``one=True`` (default), then only return the smallest
            bin that completely contains these coordinates (useful for
            assigning a single bin). If ``one=False``, then return the set of
            *all* bins that overlap these coordinates (useful for looking for
            features that could intersect)

    Returns:
        int: The calculated bin.
